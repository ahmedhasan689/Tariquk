...
<form action="{{ route('admin.update', ['id' => $admin->id]) }}" method="POST" enctype="multipart/form-data">
    ...
    <div class="form-row">
        ...
        <div class="form-group col-md-4">
            <label>الأسم</label>
            <input type="text" class="form-control @error('name') is-invalid @enderror" id="name" placeholder="أدخل الأسم" name="name" value="{{ $admin->name }}">
        </div>
    ...
    <div class="form-row">
        ...
        <div class="form-group col-md-6">
            ...
            <input type="text" class="form-control @error('phone_number') is-invalid @enderror" id="phone_number" name="phone_number" placeholder="أدخل رقم الجوال" value="{{ $admin->phone_number }}">
        </div>
        ...
        <div class="form-group col-md-6">
            <label>الايميل</label>
            <input type="email" class="form-control @error('email') is-invalid @enderror" id="Email" name="email" placeholder="أدخل الايميل" value="{{ $admin->email }}">
        </div>
    ...
    <div class="form-row">
        <!-- Country_name -->
        <div class="form-group col-md-4">
            <label for="inputState">لمدينة</label>
            <select id="inputState" class="form-control @error('city') is-invalid @enderror" name="city">
                @foreach ($cities as $city)
                <option value="{{ $city->id }}" <?php if ($admin->city->id == $city->id) { echo 'selected';} ?>>{{ $city->city_name }}</option>
                @endforeach
            </select>
        </div>
